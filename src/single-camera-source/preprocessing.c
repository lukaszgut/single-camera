...
GstElement *create_preprocessing(void)
{
    ...
    if (effect) {
        ...
        g_printerr("Failed to create effect element for preprocessing\n");
    }

    GstPad *sink_pad = gst_element_get_static_pad(effect, "sink");
    GstPad *ghost_sink = gst_ghost_pad_new("sink", sink_pad);
    gst_object_unref(sink_pad);
    gst_element_add_pad(bin, ghost_sink);

    GstPad *src_pad = gst_element_get_static_pad(effect, "src");
